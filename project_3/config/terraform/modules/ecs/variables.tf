...
variable "execution_role_arn" {
  description = "The ARN of the IAM role that allows ECS tasks to make calls to AWS services (e.g., to pull images from ECR or send logs to CloudWatch)."
  type        = string
}

variable "alb_target_group_arn" {
  description = "The ARN of the ALB target group to associate with the service. Leave null if no load balancer is used."
  type        = string
  default     = null
}
...
variable "enable_load_balancer" {
  description = "Whether to associate the ECS service with an ALB target group."
  type        = bool
}
